...
def draw_outputs(img, outputs, class_names):
  ...
  for i in range(nums):
    ...
    img = cv2.putText(img, '{} {:.2f}'.format( class_names[int(classes[i])], score[i]), (x1 + 5, y1 + 25), font, 1, color, 2)
  return img
...
output = cv2.VideoWriter("output/test_YOLO_tf.avi", fourcc, 24.0, (1280, 720))
...
while True:
  ...
  boxes, scores, classes, nums = yolo(frame) #eager mode
